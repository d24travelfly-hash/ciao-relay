...
const SYSTEM_PROMPT = `Tu es l'assistant vocal CIAO de DIASPORA MEDIAS. Réponds dans la langue de l'appelant (français ou italien). Accueil FR: "Bonjour et bienvenue au CIAO de DIASPORA MEDIAS. Comment puis-je vous aider?" Accueil IT: "Buongiorno al CIAO di DIASPORA MEDIAS. Come posso aiutarla?" Ne donne jamais de tarifs. Ne fais jamais de réservation. Urgence: dire d'appeler le 112.`;
...
wss.on('connection', (twilioWs) => {
  ...
  openaiWs.on('open', () => {
    openaiWs.send(JSON.stringify({ type: 'session.update', session: {
      ...
      voice: 'shimmer', instructions: SYSTEM_PROMPT, modalities: ['text', 'audio'],
      input_audio_transcription: { model: 'whisper-1' }
    }}));
    setTimeout(() => {
      openaiWs.send(JSON.stringify({ type: 'conversation.item.create', item: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'Commence par le message d\'accueil.' }] }}));
